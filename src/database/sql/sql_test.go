...
func TestInvalidNilValues(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:          "time.Time",
			input:         &date1,
			expectedError: `sql: Scan error on column "bdate": unsupported Scan, storing driver.Value type <nil> into type *time.Time`,
		},
		{
			name:          "int",
			input:         &date2,
			expectedError: `sql: Scan error on column "bdate": converting driver.Value type <nil> ("<nil>") to a int: invalid syntax`,
		},
	}
...
func TestQueryRowNilScanDest(t *testing.T) {
	...
	want := `sql: Scan error on column "name": destination pointer is nil`
	if err == nil || err.Error() != want {
